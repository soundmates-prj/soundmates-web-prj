...
class ApiClient {
  ...
  private async request<T>(
    ...
    try {
      ...
      if (!response.ok) {
        throw {
          message:
            (data as ApiError).message || `HTTP error ${response.status}`,
  ...
  async registerSuperAdmin(
    data: SetupRegisterRequest,
  ): Promise<SetupRegisterResponse> {
  ...
  private async registerViaForm(
    ...
    try {
      ...
      try {
        ...
        if (
          checkResponse.url.includes("station") ||
          checkResponse.url.includes("settings")
        ) {
          return {
            success: true,
      ...
      throw {
        message:
          "Unable to connect to server. Please check if AzuraCast is running.",
        type: "network",
      } as ApiError;
    }
  }
  ...
  async createStation(stationData: {
    ...
    return this.request("/api/admin/stations", {
      ...
      body: JSON.stringify({
        ...
        backend_type: stationData.backend_config?.enable_autodj
          ? "liquidsoap"
          : "none",
  ...
  async getStations(): Promise<
    Array<{ id: number; name: string; short_name: string; is_enabled: boolean }>
  > {
   ...
   */
  async getNowPlaying(
    stationIdOrShortcode: number | string,
   ...
   */
  async reloadStationConfiguration(
    stationId: number,
   ...
   */
  async controlBackend(
    stationId: number,
    action: "start" | "stop" | "reload" | "restart" | "skip" | "disconnect",
  ): Promise<{ success: boolean; message: string }> {
   ...
   */
  async getStationServiceStatus(
    stationId: number,
  ): Promise<{ backendRunning: boolean; frontendRunning: boolean }> {
  ...
  async getFiles(
    ...
    return this.request(
      `/api/station/${stationId}/files/list?${params.toString()}`,
    );
  }
   ...
   */
  async batchFilesOperation(
    stationId: number,
    operation: "delete" | "move" | "queue" | "immediate" | "reprocess",
    files: string[],
    options?: {
      directory?: string;
      playlists?: number[];
  ...
  async getPlaylists(
    ...
    Array<{ id: number; name: string; type: string; is_enabled: boolean }>
  > {
  ...
  async togglePlaylist(stationId: number, playlistId: number): Promise<void> {
    return this.request(
      `/api/station/${stationId}/playlist/${playlistId}/toggle`,
      {
        method: "PUT",
      },
    );

...
import { Loader2, Download, Volume2, Check, Wand2, PenLine, RefreshCw, Headphones, Trash2 } from 'lucide-react';
...
export function ScriptGenerateForm() {
  ...
  const handleSaveManual = async () => {
    ...
    try {
      ...
      // Build a Script-compatible object so audio generation can use it
      setGeneratedScript({
        id: result.scriptId,
        userId: '',
        topic: manualTitle.trim() || 'Script thủ công',
    ...
    <div className="script-generate-form">
      <div className="script-generate-header">
        ...
        <p className="script-generate-subtitle">
          Tạo script với AI hoặc tự biên tập nội dung của bạn
        </p>
      </div>
      ...
      {/* ── Audio Library Toggle ── */}
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 8 }}>
        <button
          type="button"
          className="btn btn-outline"
          style={{ fontSize: '0.82rem', padding: '0.4rem 0.9rem', display: 'flex', alignItems: 'center', gap: 6 }}
          onClick={() => { setShowAudioLibrary(v => !v); if (!showAudioLibrary) void handleLoadAudios(); }}
        >
          <Headphones size={14} /> {showAudioLibrary ? 'Ẩn thư viện' : 'Audio của tôi'}
        </button>
      </div>

      {/* ── Audio Library Panel ── */}
      {showAudioLibrary && (
        <div className="sgf-audio-library">
          <div className="sgf-audio-library__header">
            <span style={{ fontWeight: 700, fontSize: '0.9rem' }}>🎵 Thư viện Audio</span>
            <button
              type="button"
              className="btn btn-outline"
              style={{ fontSize: '0.78rem', padding: '0.3rem 0.7rem', display: 'flex', alignItems: 'center', gap: 4 }}
              onClick={() => void handleLoadAudios()}
              disabled={loadingAudios}
            >
              {loadingAudios ? <Loader2 size={12} className="spinner" /> : <RefreshCw size={12} />} Tải lại
            </button>
          </div>
          {loadingAudios ? (
            <div style={{ textAlign: 'center', padding: '1rem', color: 'var(--neutral-500)' }}>
              <Loader2 size={20} className="spinner" /> Đang tải...
            </div>
          ) : myAudios.length === 0 ? (
            <p style={{ color: 'var(--neutral-500)', fontSize: '0.85rem', padding: '1rem', textAlign: 'center' }}>
              Chưa có audio nào. Tạo script và generate audio để lưu ở đây.
            </p>
          ) : (
            <div className="sgf-audio-list">
              {myAudios.map((audio: any) => {
                ...
                return (
                  <div key={aid} className="sgf-audio-item">
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: 600, fontSize: '0.88rem', marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{title}</div>
                      <div style={{ fontSize: '0.75rem', color: 'var(--neutral-500)', marginBottom: 4 }}>{createdAt}</div>
                      <audio controls src={getAudioFileUrl(aid)} style={{ width: '100%', height: 32 }} preload="none" />
                    </div>
                    <button
                      type="button"
                      className="btn btn-outline"
                      style={{ flexShrink: 0, padding: '0.35rem 0.65rem', fontSize: '0.78rem', display: 'flex', alignItems: 'center', gap: 4 }}
                      onClick={() => void handleDownloadAudio(aid, audio.fileName)}
                      disabled={downloadingAudioId === aid}
                    >
                      {downloadingAudioId === aid ? <Loader2 size={12} className="spinner" /> : <Download size={12} />}
                    </button>
                    <button
                      type="button"
                      className="btn btn-outline"
                      style={{ flexShrink: 0, padding: '0.35rem 0.65rem', fontSize: '0.78rem', display: 'flex', alignItems: 'center', gap: 4, color: '#ef4444', borderColor: 'rgba(239,68,68,0.3)' }}
                      onClick={() => void handleDeleteAudio(aid)}
                      disabled={deletingAudioId === aid}
                      title="Xóa audio"
                    >
                      {deletingAudioId === aid ? <Loader2 size={12} className="spinner" /> : <Trash2 size={12} />}
                    </button>
                  </div>
                );
              })}
        ...
        <div className="script-result">
          <div className="result-header">
            <h3 className="result-title">
              {createMode === 'manual' ? '✅ Script đã lưu thành công!' : 'Script đã tạo thành công!'}
            </h3>
            <div className="result-actions">
              <button type="button" className="btn btn-outline" onClick={handleCopyScript}>Copy Script</button>
              <button type="button" className="btn btn-outline" onClick={handleReset}>Tạo script mới</button>
            </div>
          </div>
